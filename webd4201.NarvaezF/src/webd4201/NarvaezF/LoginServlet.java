		...
		try {
			...
			//set up database connection
			 ...
			 try {
				 ...
				 id = request.getParameter("Login").trim();
				 studentId = Long.parseLong(id);
				 password = request.getParameter( "Password" ).trim();
				 //check the hashpassword
				 hashPassword = User.get_SHA_1_SecurePassword(password);
				 
				 //authenticate user
				 Student aStudent = Student.authenticate(studentId, hashPassword);
			 ...
			 catch( NotFoundException nfe)
	            {
	               ...
	               if(!Student.findPassword(hashPassword))
	            	   errorBuffer.append(" \nInvalid Password.");
